...
// readSecrets reads all credentials from the Credentials audit file
func readSecrets(outDir string) ([]SecretSummary, error) {
...
// generateSecretsByServiceChart generates a bar chart showing credentials by service
func generateSecretsByServiceChart(outDir string, showLegend bool) *charts.Bar {
	credentials, err := readSecrets(outDir)
	if err != nil {
...
// generateSecretsTimelineChart generates a line chart showing credentials over time
func generateSecretsTimelineChart(outDir string, showLegend bool) *charts.Line {
	credentials, err := readSecrets(outDir)
...
// generateSecretsUsernamesChart generates a bar chart showing top usernames
func generateSecretsUsernamesChart(outDir string, showLegend bool) *charts.Bar {
	credentials, err := readSecrets(outDir)
	if err != nil {
		credentials = []SecretSummary{}
	}
...
// generateSecretsFlowsChart generates a pie chart showing credentials by flow
func generateSecretsFlowsChart(outDir string, showLegend bool) *charts.Pie {
	credentials, err := readSecrets(outDir)
	if err != nil {
		credentials = []SecretSummary{}
	}
