...
${NETCAP_BIN} capture -read "$f" \
	...
	-exclude TLSClientHello,TLSServerHello,HTTP,Connection,DeviceProfile,Host,File,POP3,Software,Service,Secret,SSH,Vulnerability,Exploit,Mail

# Include only custom decoders
${NETCAP_BIN} capture -read "$f" \
	-out "${OUT}/${file}-numcores-workers-only-customdecoders.net" \
	-include TLSClientHello,TLSServerHello,HTTP,Connection,DeviceProfile,Host,File,POP3,Software,Service,Secret,SSH,Vulnerability,Exploit,Mail
