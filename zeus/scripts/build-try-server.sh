#!/bin/bash
#
# Build and push the Netcap Try Server container
#

set -e

# Image configuration
IMAGE_NAME="netcap-try"
...
echo "[INFO] Building netcap try server container"
echo "[INFO] Image: ${IMAGE_TAG}"
echo "[INFO] Registry: ${REGISTRY}"
echo "[INFO] Version: ${VERSION}"

# Check if frontend assets exist
if [ ! -d "cmd/try/webui/frontend/out" ]; then
    echo "[WARN] Frontend assets not found at cmd/try/webui/frontend/out"
    echo "[INFO] Attempting to copy from capture webUI..."
    ...
    if [ -d "cmd/capture/webui/frontend/out" ]; then
        ...
        echo "[ERROR] Capture webUI frontend not built!"
        echo "[INFO] Please build the frontend first:"
        echo "       cd cmd/capture/webui/frontend"
        echo "       npm install && npm run build"
        echo "       cd ../../../.."
        echo "       cp -r cmd/capture/webui/frontend/out cmd/try/webui/frontend/"
        exit 1
    fi
fi
...
echo "  docker run -d -p 7070:7070 -v netcap-try-data:/data ${IMAGE_TAG}"
echo ""
