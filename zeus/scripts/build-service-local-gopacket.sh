#!/bin/bash
#
# Build and push the Netcap Service Mode container with local gopacket
#
...
echo "[INFO] Local gopacket path: ${GOPACKET_PATH}"
...
# Verify gopacket path exists
if [ ! -d "$GOPACKET_PATH" ]; then
    echo "[ERROR] Local gopacket directory not found at: $GOPACKET_PATH"
    echo "[INFO] Please set GOPACKET_PATH to the correct path:"
    echo "       GOPACKET_PATH=/path/to/gopacket-community zeus build-service-local-gopacket"
    exit 1
fi

# Get absolute path to gopacket
...
if [ ! -d "$NETCAP_ROOT/docker/service-local-gopacket" ]; then
    echo "[INFO] Creating docker/service-local-gopacket directory..."
    mkdir -p "$NETCAP_ROOT/docker/service-local-gopacket"
fi

# Create Dockerfile for local gopacket build if it doesn't exist or needs updating
cat > "$NETCAP_ROOT/docker/service-local-gopacket/Dockerfile" << 'DOCKERFILE_END'
# Netcap Try Service - Alpine Linux (with local gopacket)
# Container for running the try service with file upload capabilities (using local gopacket)
...
# Copy gopacket source FIRST (to use local version)
WORKDIR /gopacket
COPY gopacket/ .
...
# Add replace directive to use local gopacket
RUN echo "replace github.com/gopacket/gopacket => /gopacket" >> go.mod
...
LABEL description="Netcap Service Mode - Upload and analyze PCAP files (with DPI support and local gopacket)"
...
echo "[INFO] Created/updated Dockerfile at docker/service-local-gopacket/Dockerfile"

DOCKER_DIR="$NETCAP_ROOT/docker/service-local-gopacket"
...
if [[ "${USE_BUILDX}" == "true" && "${NETCAP_PUSH_IMAGES}" == "true" ]]; then
    ...
    echo "[INFO] Copying gopacket source from $GOPACKET_ABS_PATH..."
...
echo ""
echo "This container was built with local gopacket from: ${GOPACKET_ABS_PATH}"
echo ""
