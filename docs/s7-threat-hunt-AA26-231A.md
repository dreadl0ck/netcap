...
The S7CommPlus visibility fields (`PayloadObscured`, `S7PlusOpcode`, …) and the
GeoIP-on-Connection fields (`SrcGeoLocation`, `DstASN`, …) were added to
`types/netcap.proto`. The generated `types/netcap.pb.go` is tracked, but it is
**not regenerated by `go build`** — there are no `go:generate` directives, so a
proto change only reaches the types package when someone runs protoc. If you
edited the schema, regenerate before building or the new fields will not exist:

```bash
# via zeus (preferred)
zeus gen-proto-dev

# or directly (requires protoc + protoc-gen-gogofaster on PATH)
...
allowlist and temporal helpers do **not** depend on the proto regeneration and
work against the existing generated types.
...
**Service mode:** when running `net capture --service`, rule evaluation happens
in the webui/service process (post-hoc over written audit records). Provide the
allowlist to that process via the `NC_APPROVED_WORKSTATIONS` environment
variable (or the `-approved-workstations` flag on the service invocation), e.g.:

```bash
NC_APPROVED_WORKSTATIONS=approved-ews.txt net capture --service -http 127.0.0.1:8080
```

If the allowlist is not loaded, `IsApprovedWorkstation` returns false for every
source, so every negated hunt rule fires on all S7comm traffic — including
legitimate engineering-workstation activity. Load it, or expect noise.
...
Key fields for hunting (see `types/netcap.proto` `message S7Comm`):
