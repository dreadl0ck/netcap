...
`netcap.proto`. The generated `types/netcap.pb.go` is **gitignored and not
regenerated by `go build`** (there are no `go:generate` directives and no CI
regeneration step). After pulling these changes, regenerate it once before
building, or the new fields will not exist:
