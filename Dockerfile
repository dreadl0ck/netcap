...
RUN cd liblinear && make && cp linear.h /usr/local/include && cp linear.o /usr/local/lib
...
RUN ldconfig -l /usr/local/lib/liblinear
RUN ldconfig -l /go/liblinear
RUN ldconfig -l /go
RUN ldconfig -l /usr/local/lib
RUN ldconfig -p
...
RUN go build -v -mod=readonly -ldflags "-v -r /usr/local/lib -s -w -X github.com/dreadl0ck/netcap.Version=v${VERSION}" -o /netcap/net.capture -i github.com/dreadl0ck/netcap/cmd/capture
RUN echo YESSS
RUN exit 1
