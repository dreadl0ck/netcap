...
RUN apk update
RUN apk add --no-cache gcc libpcap-dev libnetfilter_queue-dev linux-headers musl-dev git vim autoconf automake libtool make g++ bison flex
...
RUN cd liblinear && make && cp linear.h /usr/local/include && cp linear.o /usr/local/lib && cp linear.o /usr/local/lib
...
ENV LDFLAGS -L/go -L/go/liblinear -L/usr/local/lib -L/lib -llinear -ltrace -lndpi -lpcap -lm -pthread -linkmode external -v
...
RUN go build -mod=readonly -ldflags "-r /usr/local/lib -s -w -X github.com/dreadl0ck/netcap.Version=v${VERSION}" -o /netcap/net.capture -i github.com/dreadl0ck/netcap/cmd/capture
