...
func sipHarvesterFunc(data []byte, ident string, ts time.Time) *types.Secret {
	if len(data) < 20 {
		return nil
	}
...
func extractSIPDigestAuth(data []byte, ident string, ts time.Time, lines [][]byte, firstLine string) *types.Secret {
...
func extractSIPBasicAuth(data []byte, ident string, ts time.Time, lines [][]byte, firstLine string) *types.Secret {
...
func extractSIPAuthResponse(data []byte, ident string, ts time.Time, lines [][]byte, firstLine string) *types.Secret {
