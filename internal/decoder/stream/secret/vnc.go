...
func vncHarvesterFunc(data []byte, ident string, ts time.Time) *types.Secret {
	if len(data) < 50 {
		return nil
	}
...
func vncARDHarvester(data []byte, ident string, ts time.Time) *types.Secret {
...
func vncMSLogonHarvester(data []byte, ident string, ts time.Time) *types.Secret {
