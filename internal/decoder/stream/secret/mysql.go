...
func mysqlHarvesterFunc(data []byte, ident string, ts time.Time) *types.Secret {
	if len(data) < 50 {
		return nil
	}
...
func mysqlOldPasswordHarvesterFunc(data []byte, ident string, ts time.Time) *types.Secret {
