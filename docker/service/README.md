# Netcap Service Mode - Docker Setup

A Docker container for running Netcap in service mode with file upload and analysis capabilities.
...
VERSION=0.7.7 docker-compose up -d
...
The Docker image version can be set with the `VERSION` environment variable:

```bash
# Default version (from docker-compose.yml)
docker-compose build

# Specific version
VERSION=0.7.7 docker-compose build
```
...
|----------|---------|-------------|
| `NC_DATA_DIR` | `/data/netcap-service` | Directory for storing uploads and results |
...
| `NC_MAX_FILE_SIZE` | `104857600` | Maximum upload file size in bytes (100MB) |
| `NC_MAX_ANALYSIS_HOUR` | `10` | Maximum analyses per hour per IP |
...
3. **Check container logs for errors:**
   ```bash
   docker-compose logs netcap-service
   ```

4. **Test from inside the container:**
   ```bash
   docker exec -it netcap-service curl http://localhost:7070/health
   ```
...
docker inspect netcap-service --format='{{json .NetworkSettings.Networks}}' | grep proxy

# Check Traefik logs
docker logs traefik | grep netcap-service

# Test from Traefik's network
docker run --rm --network=proxy curlimages/curl:latest curl http://netcap-service:7070/health
```
...
The service uses a dedicated bridge network (`netcap-service`). To connect other containers:
...
```bash
# Inspect the volume
docker volume inspect try_netcap-service-data

# Backup the volume
docker run --rm -v try_netcap-service-data:/data -v $(pwd):/backup alpine tar czf /backup/netcap-service-backup.tar.gz /data

# Restore the volume
docker run --rm -v try_netcap-service-data:/data -v $(pwd):/backup alpine tar xzf /backup/netcap-service-backup.tar.gz -C /
```
...
```bash
# Check health status
docker inspect --format='{{.State.Health.Status}}' netcap-service
```
...
```bash
docker exec -it netcap-service netcap util -download-dbs
```
...
VERSION=0.7.7 docker-compose build --no-cache
docker-compose up -d

# Or use the zeus build script (recommended)
cd ../..
zeus build-service

# Or build-service script directly
./zeus/scripts/build-service.sh
```

### Interactive Shell

```bash
docker exec -it netcap-service sh
```
