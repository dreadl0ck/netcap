...
// TestCredentialsPCAPIntegration is a comprehensive integration test that verifies
...
func TestCredentialsPCAPIntegration(t *testing.T) {
...
// TestCredentialsPCAPByProtocol runs focused tests for each protocol individually
...
func TestCredentialsPCAPByProtocol(t *testing.T) {
	pcapFile := filepath.Join("testdata", "credentials.pcap")
	if _, err := os.Stat(pcapFile); os.IsNotExist(err) {
...
// TestCredentialsPCAPAllHarvesters is skipped by default due to long runtime.
// Enable by running: go test -v -run TestCredentialsPCAPAllHarvesters -timeout 10m
// This test verifies all registered harvesters against the merged credentials.pcap
func TestCredentialsPCAPAllHarvesters(t *testing.T) {
	if testing.Short() {
