...
func matchServiceProbes(serv *service, banner []byte, ident string) {
	...
	if expectedCategory != "" {
		if probes, ok := serviceProbes[expectedCategory]; ok {
			streamLog.Info("matching probes", zap.String("ident", ident), zap.String("expectedCategory", expectedCategory))
...
func initServiceProbes() error {
		...
		if strings.HasPrefix(line, "match") {
			...
			if conf.UseRE2 {
				if _, ok := ignoredProbesRE2[ident]; ok {
					streamLog.Debug("ignoring probe", zap.String("ident", ident))
					...
					case "c":
						...
						for {
							...
							if errors.Is(err, io.EOF) {
								...
								if err != nil {
									streamLog.Error("error while parsing cpe tag for service probe",
										zap.Error(err),
	...
	streamLog.Info("loaded nmap service probes", zap.Int("total", len(serviceProbes)))
