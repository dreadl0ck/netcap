...
func decodeSMTPAuthPlain(in string) (user, pass string) {
	...
	if err != nil {
		streamLog.Warn("captured SMTP Auth Plain credentials, but could not decode them", zap.String("input", in))
...
func decodeSMTPLogin(in [][]byte, typ string) (user, pass string) {
	...
	if err != nil {
		streamLog.Warn("captured "+typ+" credentials, but could not decode them", zap.String("input", string(in[1])))
	...
	if err != nil {
		streamLog.Warn("captured credentials, but could not decode them",
...
func imapHarvester(data []byte, ident string, ts time.Time) *types.Credentials {
	...
	if len(matchesPlainSeparate) > 1 {
		...
		if err != nil {
			streamLog.Warn("captured IMAP credentials, but could not decode them",
				zap.Error(err),
...
var credentialsDecoder = NewStreamDecoder(
