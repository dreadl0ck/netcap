...
func (h *smtpReader) New(conv *conversationInfo) streamDecoder {
	return &smtpReader{
		conversation: conv,
	}
}
...
nextLine:
	...
	if errors.Is(err, io.EOF) || errors.Is(err, io.ErrUnexpectedEOF) {
		...
		streamLog.Error("SMTP Request error",
...
nextLine:
	...
	if errors.Is(err, io.EOF) || errors.Is(err, io.ErrUnexpectedEOF) {
		...
		logReassemblyError(opSMTPResponse, h.conversation.ident, err)
...
func (h *smtpReader) processSMTPConversation() (mailIDs []string) {
	...
	for {
		...
		case smtpDATA:
			...
			mail := parseMail(h.conversation, []byte(r.Data), from, to, smtpLog, serviceSMTP)
			writeMail(mail)
