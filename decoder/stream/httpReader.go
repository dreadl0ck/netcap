...
func (h *httpReader) New(conversation *conversationInfo) streamDecoder {
	return &httpReader{
...
func (h *httpReader) readResponse(b *bufio.Reader) error {
	...
	if errors.Is(err, io.EOF) || errors.Is(err, io.ErrUnexpectedEOF) {
		...
		logReassemblyError("HTTP-response", h.conversation.ident, err)
		return err
	}
	...
	s := len(body)
	if err != nil {
		logReassemblyError("HTTP-response-body", fmt.Sprintf("%s: failed to get body(parsed len:%d)", h.conversation.ident, s), err)
	} else {
		_ = res.Body.Close()
	...
	if (err == nil || conf.WriteIncomplete) && conf.FileStorage != "" {
		...
		// save file to disk
		return saveFile(h.conversation, source, name, err, body, encoding, ctype, host)
	}
...
func (h *httpReader) readRequest(b *bufio.Reader) error {
	...
	if errors.Is(err, io.EOF) || errors.Is(err, io.ErrUnexpectedEOF) {
		...
		logReassemblyError("HTTP-request", h.conversation.ident, err)
		return err
	}
	...
	err = req.ParseForm()
	if err != nil {
		logReassemblyError("HTTP-request", fmt.Sprintf("%s: failed to parse form values", h.conversation.ident), err)
	}
