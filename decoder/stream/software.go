...
func newSoftware(i *decoderutils.PacketInfo) *software {
	return &software{
		Software: &types.Software{
			Timestamp: i.Timestamp,
		},
...
var softwareDecoder = NewStreamDecoder(
	...
	func(d *Decoder) error {
		...
		streamLog.Info("loaded HASSH digests", zap.Int("total", len(hashDBMap)))
		...
		streamLog.Info("loaded CMS db", zap.Int("total", len(cmsDB)))
		...
		streamLog.Info("loaded Ja3/ja3S database", zap.Int("total_records", len(ja3db.Servers)))

		return nil
	},
	nil,
	func(e *Decoder) error {
	...
	for framework := range jsonParsed.ChildrenMap() {
		...
		if s, ok := jsonParsed.Path(framework + ".headers").Data().(map[string]interface{}); ok {
			...
			for name, re := range s {
				...
				if err != nil {
					streamLog.Info("failed to compile regex from CMS db HEADER",
						zap.Error(err),
		...
		if s, ok := jsonParsed.Path(framework + ".cookies").Data().(map[string]interface{}); ok {
			...
			for name, re := range s {
				...
				r, err := regexp.Compile(fmt.Sprint(re))
				if err != nil {
					streamLog.Info("failed to compile regex from CMS db COOKIE",
						zap.Error(err),
						zap.String("re", fmt.Sprint(re)),
						zap.String("framework", framework),
					)
