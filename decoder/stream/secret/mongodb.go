...
func mongodbHarvesterFunc(data []byte, ident string, ts time.Time) *types.Secret {
	if len(data) < 50 {
		return nil
	}
...
func mongodbChallengeResponseHarvesterFunc(data []byte, ident string, ts time.Time) *types.Secret {
...
func mongodbPlaintextHarvesterFunc(data []byte, ident string, ts time.Time) *types.Secret {
