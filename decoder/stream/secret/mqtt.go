...
func mqttHarvesterFunc(data []byte, ident string, ts time.Time) *types.Secret {
	if len(data) < 4 {
		return nil
	}
...
func parseMQTTConnect(data []byte, ident string, ts time.Time) *types.Secret {
	if len(data) < 2 {
		return nil
	}
...
func parseMQTTConnack(data []byte, ident string, ts time.Time) *types.Secret {
	if len(data) < 4 {
		return nil
	}
