...
func socksHarvesterFunc(data []byte, ident string, ts time.Time) *types.Secret {
	if len(data) < 3 {
		return nil
	}
...
func parseSOCKS5UserPass(data []byte, ident string, ts time.Time) *types.Secret {
...
func parseSOCKS4Request(data []byte, ident string, ts time.Time) *types.Secret {
...
func parseSOCKS5AuthResponse(data []byte, ident string, ts time.Time) *types.Secret {
	if len(data) != 2 || data[0] != socksUserPassVersion {
		return nil
	}
