...
// secretHarvester is a function that takes the data of a bi-directional network stream over TCP
...
type secretHarvester func(data []byte, ident string, ts time.Time) *types.Secret

// Harvester represents a credential harvester with its function and metadata
type Harvester struct {
	Name          string
	Description   string
	HarvesterFunc secretHarvester
}
...
// ResetSecretStore clears the credentials deduplication store
