...
type httpMetaStore struct {
	...
	// TODO: cookies are not used for software detection at the moment
...
func updateHTTPStore(h *types.HTTP) {
	...
	for key, val := range h.ResponseHeader {
		if _, ok := cmsHeadersList[key]; ok {
			httpStore.CMSHeaders[h.DstIP] = append(httpStore.CMSHeaders[h.DstIP], headerForApps{HeaderName: key, HeaderValue: val})
		}
	}
	...
	if toSet, ok := h.ResponseHeader["Set-Cookie"]; ok {
		...
		if _, ok = cmsCookiesList[cookieKey]; ok {
