...
//goland:noinspection GoUnnecessarilyExportedIdentifiers
type DNSTransformationFunc = func(lt LocalTransform, trx *Transform, dns *types.DNS, min, max uint64, path string, ip string)
...
func DNSTransform(count DNSCountFunc, transform DNSTransformationFunc, continueTransform bool) {
	...
	if !strings.HasPrefix(filepath.Base(path), "DNS.ncap") {
		path = filepath.Join(filepath.Dir(path), "DNS.ncap.gz")
	}
	...
	for {
		...
		transform(lt, &trx, dns, min, max, path, ipaddr)
	}
