...
//goland:noinspection GoUnnecessarilyExportedIdentifiers
type IGMPTransformationFunc = func(lt LocalTransform, trx *Transform, igmp *types.IGMP, min, max uint64, path string, ip string)
...
func IGMPTransform(count IGMPCountFunc, transform IGMPTransformationFunc, continueTransform bool) {
	...
	if !strings.HasPrefix(filepath.Base(path), "IGMP.ncap") {
		path = filepath.Join(filepath.Dir(path), "IGMP.ncap.gz")
	}
	...
	if header.Type != types.Type_NC_IGMP {
		die("file does not contain IGMP records", header.Type.String())
	}
	...
	for {
		...
		transform(lt, &trx, igmp, min, max, path, ipaddr)
	}
