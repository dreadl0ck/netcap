...
//goland:noinspection GoUnnecessarilyExportedIdentifiers
type DHCPV4TransformationFunc = func(lt LocalTransform, trx *Transform, dhcp *types.DHCPv4, min, max uint64, path string, ip string)
...
func DHCPV4Transform(count DHCPCountFunc, transform DHCPV4TransformationFunc, continueTransform bool) {
	...
	if header != nil && header.Type != types.Type_NC_DHCPv4 {
		die("file does not contain DHCP records", header.Type.String())
	}
